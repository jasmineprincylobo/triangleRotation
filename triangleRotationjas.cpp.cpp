// triangleRotationjas.cpp.cpp : Defines the entry point for the console application.
//
...
void display()
{
float rotO[3][3] ={{cos(theta), -sin(theta), 0},{sin(theta), cos(theta),
0},{0, 0, 1}};
float rotF[3][3] ={{cos(theta), -sin(theta),
xc-xc*cos(theta)+yc*sin(theta)},{sin(theta), cos(theta),
yc-yc*cos(theta)-xc*sin(theta)},{0, 0, 1}};
glClear(GL_COLOR_BUFFER_BIT);
 //x-axis and y-axis line
 glBegin(GL_LINES);
 glVertex2f(0,-60);
glVertex2f(0,60);
 glVertex2f(-60,0);
 ...
 glColor3f(1,0,0);
 triangle(tri);
 //Rotation of triangle about an origin
 glColor3f(0,0,1);
 multiply(rotO);
 //Rotation of triangle about a fixed point
 glColor3f(0,1,0);
 multiply(rotF);
glFlush();
}
